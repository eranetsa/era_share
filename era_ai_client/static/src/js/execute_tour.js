...
registry.category("actions").add("execute_custom_tour", async (env, params) => {
    ...
    try {
        ...
        // Monitor console log for "tour succeeded"
        ...
        if (tourmode === "auto") {
            ...
            await env.services.tour_service.startTour(tourName, { mode: "manual", redirect: false });
            console.log("Tour executed in Manual mode.");

        }

        // Check if the tour was consumed
        let listTour = await env.services.orm.call("web_tour.tour", "get_current_tour", []);
        if (listTour.name == tourName) {
        ...
        env.services.notification.add(_t("Failed to execute the tour.\n Please wait will try again."), { type: "warning" });
        try{
            const response = await env.services.orm.call(
                'tour.connector',
                'chat_with_gpt', // 'get_chatgpt_response',
                [tour_connector],
                    {});

            env.services.action.doActionButton({
                type: "object",
