            ...
            if data_list['code'] == '200':
                for data in data_list['data']:
                    ...
                    def convert_time_to_seconds(time_str):
                        match = re.match(r'(\d+)س\s+(\d+)د\s+(\d+)', time_str)
                        if match:
                    ...
                    # Check if 'size' is a time string and convert
                    size_value = data.get("size", 0)
                    if isinstance(size_value, str):
                        size_value = convert_time_to_seconds(size_value)

                    # Check if 'days' is a time string and convert
                    days_value = data.get("days", 0)
                    if isinstance(days_value, str):
                        days_value = convert_time_to_seconds(days_value)
